...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, Runnable
{
    ...
    private boolean configureTransitions(final int n, final TransitionState transitionState, final boolean b, final SparseArray<Fragment> sparseArray, final SparseArray<Fragment> sparseArray2) {
        ...
        if (viewGroup == null) {
            ...
            if (enterTransition == null && o == null && exitTransition == null) {
                ...
                final Object mergeTransitions = FragmentTransitionCompat21.mergeTransitions(enterTransition, captureExitingViews, o, b3);
                if (mergeTransitions != null) {
                    FragmentTransitionCompat21.addTransitionTargets(enterTransition, o, captureExitingViews, (View)viewGroup, (FragmentTransitionCompat21.ViewRetriever)viewRetriever, transitionState.nonExistentView, transitionState.enteringEpicenterView, transitionState.nameOverrides, list3, list2, map, arrayMap, list);
    ...
    private static ArrayMap<String, View> remapNames(final ArrayList<String> list, final ArrayList<String> list2, final ArrayMap<String, View> arrayMap) {
        ...
        if (arrayMap.isEmpty()) {
            ...
            final ArrayMap<String, View> arrayMap3 = new ArrayMap<String, View>();
            for (int size = list.size(), i = 0; i < size; ++i) {
                final View view = arrayMap.get(list.get(i));
                if (view != null) {
                    arrayMap3.put(list2.get(i), view);
                }
            }
            arrayMap2 = arrayMap3;
        }
    ...
    public void run() {
        ...
        final TransitionState transitionState = beginTransition = null;
        if (BackStackRecord.SUPPORTS_TRANSITIONS) {
            beginTransition = transitionState;
            if (this.mManager.mCurState >= 1) {
        ...
        for (Op op = this.mHead; op != null; op = op.next) {
            ...
            switch (op.cmd) {
                ...
                case 2: {
                    ...
                    if (this.mManager.mAdded != null) {
                        int index = this.mManager.mAdded.size() - 1;
                        while (true) {
                            fragment3 = fragment2;
                            if (index < 0) {
                                break;
                            }
                            final Fragment obj = this.mManager.mAdded.get(index);
                            if (FragmentManagerImpl.DEBUG) {
                                Log.v("FragmentManager", "OP_REPLACE: adding=" + fragment2 + " old=" + obj);
                            }
                            Fragment fragment4 = fragment2;
                            if (obj.mContainerId == mContainerId) {
                                if (obj == fragment2) {
                                    fragment4 = null;
                                    op.fragment = null;
                                }
                                    ...
                                    this.mManager.removeFragment(obj, mTransition, mTransitionStyle);
                                    fragment4 = fragment2;
                                }
                            }
                            --index;
