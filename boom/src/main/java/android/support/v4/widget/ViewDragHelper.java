...
public class ViewDragHelper
{
    ...
    private boolean checkNewEdgeDrag(float abs, float abs2, final int n, final int n2) {
        ...
        abs2 = Math.abs(abs2);
        boolean b2 = b;
        if ((this.mInitialEdgesTouched[n] & n2) == n2) {
            b2 = b;
            if ((this.mTrackingEdges & n2) != 0x0) {
                ...
                if ((this.mEdgeDragsLocked[n] & n2) != n2) {
                    ...
                    if ((this.mEdgeDragsInProgress[n] & n2) != n2) {
                        ...
                        else if (abs < 0.5f * abs2 && this.mCallback.onEdgeLock(n2)) {
    ...
    private int computeAxisDuration(int min, int abs, final int n) {
        if (min == 0) {
            min = 0;
        }
        else {
            ...
            final float min2 = Math.min(1.0f, Math.abs(min) / (float)width);
            final float n3 = (float)n2;
            final float n4 = (float)n2;
            final float distanceInfluenceForSnapDuration = this.distanceInfluenceForSnapDuration(min2);
            abs = Math.abs(abs);
            if (abs > 0) {
                min = Math.round(1000.0f * Math.abs((n3 + n4 * distanceInfluenceForSnapDuration) / abs)) * 4;
            }
            else {
                min = (int)((Math.abs(min) / (float)n + 1.0f) * 256.0f);
            }
    ...
    private int computeSettleDuration(final View view, int computeAxisDuration, int computeAxisDuration2, int clampMag, int abs) {
        ...
        abs = Math.abs(computeAxisDuration);
