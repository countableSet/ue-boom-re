...
public class DrawerLayout extends ViewGroup implements DrawerLayoutImpl
{
    ...
    public boolean isDrawerOpen(final View obj) {
        ...
        if ((((LayoutParams)obj.getLayoutParams()).openState & 0x1) != 0x1) {
            b = false;
        }
        return b;
    }
    ...
    void updateDrawerState(int viewDragState, int i, final View view) {
        final int viewDragState2 = this.mLeftDragger.getViewDragState();
        viewDragState = this.mRightDragger.getViewDragState();
        if (viewDragState2 == 1 || viewDragState == 1) {
            viewDragState = 1;
        }
        else if (viewDragState2 == 2 || viewDragState == 2) {
            viewDragState = 2;
        }
        else {
            viewDragState = 0;
    ...
    private class ViewDragCallback extends Callback
    {
        ...
        @Override
        public int clampViewPositionHorizontal(final View view, int n, int width) {
            if (DrawerLayout.this.checkDrawerViewAbsoluteGravity(view, 3)) {
                n = Math.max(-view.getWidth(), Math.min(n, 0));
            }
            else {
                width = DrawerLayout.this.getWidth();
                n = Math.max(width - view.getWidth(), Math.min(n, width));
            }
            return n;
        }
