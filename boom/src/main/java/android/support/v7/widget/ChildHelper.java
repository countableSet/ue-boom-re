...
class ChildHelper
{
    ...
    View findHiddenNonRemovedView(final int n, final int n2) {
        for (int size = this.mHiddenViews.size(), i = 0; i < size; ++i) {
            ...
            if (childViewHolder.getLayoutPosition() == n && !childViewHolder.isInvalid() && !childViewHolder.isRemoved()) {
                View view2 = view;
                if (n2 != -1) {
                    if (childViewHolder.getItemViewType() != n2) {
                        continue;
                    }
                    view2 = view;
                }
    ...
    static class Bucket
    {
        ...
        int countOnesBefore(int n) {
            if (this.next == null) {
                if (n >= 64) {
                    n = Long.bitCount(this.mData);
                }
                else {
                    n = Long.bitCount(this.mData & (1L << n) - 1L);
                }
            }
            else if (n < 64) {
                n = Long.bitCount(this.mData & (1L << n) - 1L);
                ...
                n = this.next.countOnesBefore(n - 64) + Long.bitCount(this.mData);
            }
            return n;
        }
