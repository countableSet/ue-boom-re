...
public class ListPopupWindow
{
    ...
    static {
        ...
        //  20     51     64     76     Ljava/lang/NoSuchMethodException;
        // 
    ...
    private int buildDropDown() {
        ...
        if (this.mDropDownAlwaysVisible || this.mDropDownHeight == -1) {
            n6 = maxAvailableHeight + n4;
        }
        else {
            int n7 = 0;
            switch (this.mDropDownWidth) {
                default: {
                    n7 = View$MeasureSpec.makeMeasureSpec(this.mDropDownWidth, 1073741824);
                    break;
                }
                case -2: {
                    n7 = View$MeasureSpec.makeMeasureSpec(this.mContext.getResources().getDisplayMetrics().widthPixels - (this.mTempRect.left + this.mTempRect.right), Integer.MIN_VALUE);
                    break;
                }
                case -1: {
                    n7 = View$MeasureSpec.makeMeasureSpec(this.mContext.getResources().getDisplayMetrics().widthPixels - (this.mTempRect.left + this.mTempRect.right), 1073741824);
                    break;
                }
            }
            final int measureHeightOfChildrenCompat = this.mDropDownList.measureHeightOfChildrenCompat(n7, 0, -1, maxAvailableHeight - n2, -1);
            int n8 = n2;
            if (measureHeightOfChildrenCompat > 0) {
                n8 = n2 + n4;
            }
            n6 = measureHeightOfChildrenCompat + n8;
        }
        return n6;
    }
    
    private int getMaxAvailableHeight(final View view, int i, final boolean b) {
        if (ListPopupWindow.sGetMaxAvailableHeightMethod == null) {
            return this.mPopup.getMaxAvailableHeight(view, i);
        }
        try {
            i = (int)ListPopupWindow.sGetMaxAvailableHeightMethod.invoke(this.mPopup, view, i, b);
            return i;
        }
        catch (Exception ex) {
            Log.i("ListPopupWindow", "Could not call getMaxAvailableHeightMethod(View, int, boolean) on PopupWindow. Using the public version.");
        }
        i = this.mPopup.getMaxAvailableHeight(view, i);
        return i;
    }
    ...
    public boolean onKeyDown(final int n, final KeyEvent keyEvent) {
        ...
        if ((b2 && n == 19 && selectedItemPosition <= lookForSelectablePosition) || (!b2 && n == 20 && selectedItemPosition >= lookForSelectablePosition2)) {
            ...
            else if (b2 || n != 19 || selectedItemPosition != lookForSelectablePosition) {
                return false;
            }
        }
        return b;
    ...
    public boolean onKeyUp(final int n, final KeyEvent keyEvent) {
        ...
        if (this.isShowing() && this.mDropDownList.getSelectedItemPosition() >= 0) {
            final boolean b = onKeyUp = this.mDropDownList.onKeyUp(n, keyEvent);
            if (b) {
                onKeyUp = b;
                if (isConfirmKey(n)) {
                    this.dismiss();
